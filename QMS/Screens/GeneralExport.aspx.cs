...
namespace Screens
{
    public partial class GeneralExport : Page
    {
        ...
        /* private void GetLocal()//Infinite loop - Commented by Victor
    {
        string deleteCalled = "DELETE FROM ExportCalled";
    ...
    private int SaveLocal(DataSet objdatasetdto)
    {
        ...
        {
            //access db
            OleDbConnection conn = new OleDbConnection("Provider=Microsoft.Jet.OLEDB.4.0;data source= F:\\LED\\Export\\Pop.mdb");
    ...
    private int SaveCalled(DataSet objdatasetdto)
    {
        ...
        for (int i = 0; i < objdatasetdto.Tables[0].Rows.Count; i++)
        {
            string query0 = "INSERT INTO ExportCalled([LO_NO],[Registration],[No]) values('" + objdatasetdto.Tables[0].Rows[i]["LO_NO"].ToString() + "','" + objdatasetdto.Tables[0].Rows[i]["Registration"].ToString() + "','" + objdatasetdto.Tables[0].Rows[i]["Queue_No"].ToString() + "');";
