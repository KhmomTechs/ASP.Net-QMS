...
namespace Screens
{
    public partial class GeneralLocal : Page
    {
        ...
        private void LED()
        {
            ...
            var package = app.LoadPackage(@"F:\KPC Q-MS\SSIS Packages\Local.dtsx", null);
    ...
    {
        SqlConnection objcon = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["SecureConnectionString"].ToString());

        for (int i = 0; i < objdatasetdto.Tables[0].Rows.Count; i++)
        {
            //access db
            OleDbConnection conn = new OleDbConnection("Provider=Microsoft.Jet.OLEDB.4.0;data source= F:\\LED\\Local\\Pop.mdb");
            ...
            comm.CommandText = "INSERT INTO Called ([No], Registration, [LO_NO]) values (" + objdatasetdto.Tables[0].Rows[i]["Queue_No"] + ", '" + objdatasetdto.Tables[0].Rows[i]["Registration"].ToString() + "', " + objdatasetdto.Tables[0].Rows[i]["LO_NO"] + ")";
    ...
    private int SaveCalled(DataSet objdatasetdto)
    {
        ...
        for (int i = 0; i < objdatasetdto.Tables[0].Rows.Count; i++)
        {
            string query0 = "INSERT INTO LocalCalled([LO_NO],[Registration],[No]) values('" + objdatasetdto.Tables[0].Rows[i]["LO_NO"].ToString() + "','" + objdatasetdto.Tables[0].Rows[i]["Registration"].ToString() + "','" + objdatasetdto.Tables[0].Rows[i]["Queue_No"].ToString() + "');";
            SqlDataAdapter objda0 = new SqlDataAdapter(query0, objcon);
            objcon.Open();
        ...
        private void GetLocal()
        {
            var deleteCalled = "DELETE FROM LocalCalled";
            ...
            var adapterDb =
                new SqlDataAdapter(
                    "SELECT NewQueue.LO_NO, NewQueue.Registration, NewQueue.Queue_No FROM NewQueue INNER JOIN Operations ON NewQueue.QueueID = Operations.QueueID INNER JOIN SafetySecurity_B ON NewQueue.QueueID = SafetySecurity_B.QueueID WHERE (SafetySecurity_B.Failed IS NULL) AND (NewQueue.Finished IS NULL) AND (Operations.Called = 'True') AND (Operations.Cleared IS NULL) AND (SafetySecurity_B.EntryStatus IS NULL) AND (NewQueue.Archived IS NULL) AND (NewQueue.Status = 'CALLOD') AND (NewQueue.Withdrawal IS NULL OR NewQueue.Withdrawal = 'False') AND (NewQueue.Type = 'Local') AND (NewQueue.EnterTime = '" +
        ...
        private int SaveCalled(DataSet objdatasetdto)
        {
            ...
            for (var i = 0; i < objdatasetdto.Tables[0].Rows.Count; i++)
            {
                var query0 = "INSERT INTO LocalCalled([LO_NO],[Registration],[No]) values('" +
                             objdatasetdto.Tables[0].Rows[i]["LO_NO"] + "','" +
                             objdatasetdto.Tables[0].Rows[i]["Registration"] + "','" +
                             objdatasetdto.Tables[0].Rows[i]["Queue_No"] + "');";
        ...
        protected void GridLocal_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                if (e.Row.Cells[3].Text.Trim().Equals("CALLOD"))
                {
                    ...
                    try
                    {
                        ...
                        using (var conn = new SqlConnection(strConnection))
                        {
                            ...
                            var compareId = e.Row.Cells[6].Text.Trim();
                ...
                //check withdrawal
                if (e.Row.Cells[9].Text.Trim().Equals("True"))
                {
