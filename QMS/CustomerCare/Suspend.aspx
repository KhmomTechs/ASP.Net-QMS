<%@ Page Title="" Language="C#" MasterPageFile="~/Master/CustomerCare.master" AutoEventWireup="true" CodeFile="Suspend.aspx.cs" Inherits="CustomerCare.CustomerCare_Suspend" %>
...
<div style="margin: 0px; width: 1400px;">
    <h3>Customer care-Truck suspension</h3>
</div>
...
<asp:SqlDataSource ID="SqlLocalSource" runat="server"
                   ...
                   SelectCommand="SELECT NewQueue.*, CustomerCare.* FROM NewQueue INNER JOIN CustomerCare ON NewQueue.QueueID = CustomerCare.QueueID WHERE (NewQueue.Type = 'Local') AND (NewQueue.Archived IS NULL) AND (NewQueue.Finished IS NULL) AND (NewQueue.Rejected IS NULL) AND SetDate = @TheDate ORDER BY NewQueue.Queue_No">
    <SelectParameters>
...
<asp:SqlDataSource ID="SqlExportSource" runat="server"
                   ConnectionString="<%$ ConnectionStrings:SecureConnectionString %>"
                   SelectCommand="SELECT NewQueue.*, CustomerCare.* FROM NewQueue INNER JOIN CustomerCare ON NewQueue.QueueID = CustomerCare.QueueID WHERE (NewQueue.Type = 'Export') AND (NewQueue.Archived IS NULL) AND (NewQueue.Finished IS NULL) AND (NewQueue.Rejected IS NULL) AND SetDate = @TheDate ORDER BY NewQueue.Queue_No">
    <SelectParameters>
        <asp:Parameter Name="TheDate" Type="DateTime"/>
    </SelectParameters>
</asp:SqlDataSource>
